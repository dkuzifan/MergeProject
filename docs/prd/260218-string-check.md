...
- [ ] **번역 자동 입력**: 한국어(B열)만 입력되고 해당 행의 다른 언어 컬럼이 비어있는 경우 — 행 단위 번역 버튼 + 선택 번역 버튼 + 전체 일괄 번역 버튼 제공
  - 번역 전 파일 내 동일 한국어 원문 행을 먼저 탐색하여 이미 번역된 값을 재사용 (API 호출 없이 즉시 채움)
  - 재사용으로 채울 수 없는 언어만 Gemini API 호출
- [ ] **선택 번역**: 번역 대상 행에 체크박스를 제공하여 원하는 행만 선택하여 번역 가능
  - 헤더 체크박스로 번역 대상 전체 선택/해제
  - 1개 이상 선택 시 툴바에 "선택 번역 (N행)" 버튼 표시
  - 선택 번역 시 확인 다이얼로그 없이 즉시 처리 (사용자가 명시적으로 선택한 항목이므로)
  - 시트 탭 전환 시 선택 초기화
...
**번역**
- 번역 대상 행(한국어만 있는 행): 체크박스 표시 + 행 우측 `[번역]` 버튼 표시
- 헤더 체크박스: 번역 대상 전체 선택/해제 (indeterminate 상태 지원)
- 1개 이상 선택 시 툴바에 `[선택 번역 (N행)]` 버튼(초록) 표시 → 확인 없이 즉시 처리
