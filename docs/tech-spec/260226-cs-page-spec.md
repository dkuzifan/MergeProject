...
- **신규** `POST /api/cs/submit`
  - `translate/route.ts`와 동일한 auth 패턴: `NODE_ENV !== 'development'`일 때만 Supabase Auth 확인
  - `cs_submissions` 테이블 insert
...
- **신규 테이블** `public.cs_submissions`
  ...
  - RLS: authenticated 유저만 INSERT 허용
...
- [ ] 카테고리·아티클·문의 토픽 등 CS에서 쓰이는 데이터 형태(타입) 정의
- [ ] 6개 카테고리 목록 작성 (이름·설명·아이콘·URL 슬러그)
- [ ] 더미 아티클 30개 작성 (카테고리별 5개) + 키워드 검색 함수 구현

**Phase B: CS 페이지 레이아웃 분리** ― CS 페이지에서 기존 사이트의 하단바가 나오지 않도록 처리합니다.
- [ ] CS 경로(`/cs/...`)에서는 기존 글로벌 하단바 숨김, 여백(padding) 제거
- [ ] 완료 후 확인: 기존 페이지(스트링 체크 등)에서 하단바가 여전히 정상 표시되는지 검증

**Phase C: CS 화면 구성 요소** ― 메인 페이지를 구성하는 각 UI 조각을 만듭니다.
- [ ] CsHeader: 상단 히어로 배너 (그라디언트 배경 + 타이틀)
- [ ] SearchBar: 검색어 입력 후 Enter/버튼 클릭 시 검색 결과 페이지로 이동
- [ ] TopicCard / TopicGrid: 카테고리 카드 1개 + 2×3 그리드 레이아웃
- [ ] WhatsNew 섹션: "What's New?" 더미 카드 6개 (2×3 그리드)

**Phase D: CS 하단바 + 모달** ― CS 전용 하단 영역과 팝업 3종을 만듭니다. (C와 병행 가능)
- [ ] Language 모달: 언어 목록 팝업 (실제 언어 전환 없음, 더미 UI)
- [ ] Cookie 모달: 쿠키 설정 토글 팝업 (더미 UI)
- [ ] Accessibility 드로어: 우측에서 슬라이드로 열리는 접근성 설정 패널 (더미 UI)
- [ ] CsFooter: 위 3개 팝업을 열고 닫는 버튼이 있는 CS 전용 하단바 조립

**Phase E: CS 페이지 라우트 연결** ― C·D가 완성된 구성 요소들을 실제 URL에 붙입니다.
- [ ] CS 레이아웃(`/cs/layout.tsx`): 모든 CS 페이지 하단에 CsFooter 자동 삽입
- [ ] `/cs` 메인 페이지: 히어로·검색·카테고리·뉴스 섹션 조합
- [ ] `/cs/[카테고리]` 페이지: 유효 카테고리면 아티클 목록, 없는 카테고리면 404
- [ ] `/cs/refund-guides` 페이지: 환불 안내 Q&A 정적 페이지
- [ ] `/cs/search` 페이지: URL의 검색어를 읽어 아티클 검색 결과 표시

**Phase F: 문의 폼(Contact Us)** ― E 완료 후 진행. DB 테이블 생성을 먼저 수동으로 실행해야 합니다.
- [ ] **[수동 작업]** Supabase 대시보드에서 `cs_submissions` 테이블 생성 SQL 실행
- [ ] `/api/cs/submit` API: 폼 데이터를 받아 로그인 여부 확인 후 DB에 저장
- [ ] ContactForm 컴포넌트: 입력 유효성 검증, 이메일 일치 확인, 파일 첨부(파일명만), 제출 후 완료 메시지
- [ ] `/cs/contact` 페이지: ContactForm 삽입

**Phase G: 최종 검증**
- [ ] 빌드 오류 없음 확인 (`next build`)
- [ ] 아래 테스트 계획의 모든 시나리오 수동 확인
...
- [ ] 로그인 페이지(`/login`) 접속 → 화면 깨짐 없음
- [ ] 스트링 체크 페이지(`/string-check`) 접속 → 상단 네비바·하단 푸터 정상 표시
- [ ] 네비바에서 P32, 도구, pAI 메뉴 클릭 → 각 페이지 정상 이동
- [ ] 다크모드 전환 → 기존 페이지 색상 깨짐 없음

**2. CS 메인 페이지 확인** (`/cs`)
- [ ] 접속 시 히어로 배너·검색바·카테고리 카드 6개·뉴스 카드 6개 모두 보임
- [ ] 하단에 기존 사이트 푸터가 **없고**, CS 전용 푸터가 보임
- [ ] 다크모드로 전환해도 CS 페이지 UI가 깨지지 않음

**3. 검색 기능 확인**
- [ ] 검색바에 `"로딩"` 입력 후 Enter → 검색 결과 페이지 이동, 관련 아티클 1건 이상 표시
- [ ] 검색바에 `"xyzabc999"` 입력 → 결과 없음 메시지(`"No results for 'xyzabc999'"`) 표시
- [ ] 검색바 아무것도 안 입력하고 Enter → 페이지 이동 없음

**4. 카테고리 탐색 확인**
- [ ] `"How to Play"` 카테고리 카드 클릭 → `/cs/how-to-play` 페이지, 아티클 5개 표시
- [ ] 브라우저 주소창에 `/cs/없는카테고리` 직접 입력 → 404 페이지

**5. 문의 폼 확인** (`/cs/contact`)
- [ ] 폼 접속 시 날짜 칸에 오늘 날짜 자동 입력, 제출 버튼 비활성화 상태
- [ ] 이메일 확인 칸에 다른 이메일 입력 후 다른 곳 클릭 → 빨간색 오류 메시지 표시
- [ ] 모든 칸 입력 + 이메일 두 칸 일치 → 제출 버튼 활성화
- [ ] 제출 버튼 클릭 → 로딩 표시 → Supabase 대시보드에서 `cs_submissions` 테이블에 새 행(row) 생성 확인
- [ ] 제출 완료 → "Thank you" 완료 메시지로 폼이 교체됨
- [ ] `"Documentation required for refund"` 링크 클릭 → 환불 안내 페이지로 이동

**6. 나머지 페이지·기능 확인**
- [ ] `/cs/refund-guides` 접속 → 환불 Q&A 5개 정상 표시
- [ ] CS 하단바 `"Language"` 버튼 → 언어 선택 팝업 열림·닫힘
- [ ] CS 하단바 `"Cookie Settings"` 버튼 → 쿠키 설정 팝업 열림, 토글 동작
- [ ] CS 하단바 `"Accessibility"` 버튼 → 우측에서 패널 슬라이드로 열림·닫힘
